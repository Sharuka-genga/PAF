        ...
        <div className="fixed left-0 top-0 h-screen w-72 bg-[#0A0F1D] border-r border-[rgba(255,255,255,0.08)] z-50">
          <div className="flex flex-col h-full">
            ...
            <nav className="flex-1 p-3 space-y-1">
              ...
              {[
                { title: 'Dashboard', icon: <FiHome className="w-4 h-4" />, path: '/dashboard' },
                ...
                { title: 'Resources', icon: <FiGrid className="w-4 h-4" />, path: '/resources' },
                { title: 'Settings', icon: <FiSettings className="w-4 h-4" />, path: '/settings' },
              ].map((item) => (
                <a
                  key={item.title}
                  href={item.path}
